...
// civo-volume
type CivoVolumeSpecInput struct {
	...
	Region string `json:"region" jsonschema:"required,enum=lon1|lon2|fra1|nyc1|phx1|mum1,The Civo region where the volume will be created. Must match the region of any instance that will attach to this volume."`
	...
	SnapshotId string `json:"snapshot_id,omitempty" jsonschema:"An optional snapshot ID or reference to create this volume from. If provided; the new volume will be created from the given snapshot (inheriting its region and minimum size)."`
...
func (s *CivoVolumeSpecInput) validate() error {
	if s.VolumeName == "" {
...
func (s *CivoVolumeSpecInput) applyDefaults() {
}

func (s *CivoVolumeSpecInput) toMap() map[string]any {
...
// ParseCivoVolume validates and normalizes a CivoVolume cloud_object.
func ParseCivoVolume(cloudObject map[string]any) (*structpb.Struct, error) {
	if err := parse.ValidateHeader(cloudObject, "civo.openmcf.org/v1", "CivoVolume"); err != nil {
		return nil, err
	}
