...
// GetLogs collects Tekton task log entries for a pipeline by internally
...
func GetLogs(ctx context.Context, serverAddress, pipelineID string) (string, error) {
	return domains.WithConnection(ctx, serverAddress,
		func(ctx context.Context, conn *grpc.ClientConn) (string, error) {
			...
			stream, err := client.GetLogStream(streamCtx, &pipelinev1.PipelineId{Value: pipelineID})
			if err != nil {
				return "", domains.RPCError(err, fmt.Sprintf("pipeline logs %q", pipelineID))
			}
