...
// SearchInput holds the validated parameters for searching variable entries.
...
// Search queries variable entries across all variables groups in an org via
// the ServiceHubSearchQueryController.SearchVariables RPC.
//
// Results are individual variable entries (not whole groups), each annotated
...
func Search(ctx context.Context, serverAddress string, input SearchInput) (string, error) {
	...
	return domains.WithConnection(ctx, serverAddress,
		func(ctx context.Context, conn *grpc.ClientConn) (string, error) {
			...
			resp, err := client.SearchVariables(ctx, req)
			if err != nil {
				return "", domains.RPCError(err, fmt.Sprintf("variable entries in org %q", input.Org))
			}
