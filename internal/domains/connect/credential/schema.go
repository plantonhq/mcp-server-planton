...
const schemaScheme = "credential-schema"

// registryEntry mirrors the JSON structure in credentials/registry.json.
...
type registryData struct {
	Credentials map[string]registryEntry `json:"credentials"`
}
...
func loadRegistry() (map[string]registryEntry, error) {
	registryOnce.Do(func() {
		data, err := schemas.CredentialFS.ReadFile("credentials/registry.json")
		if err != nil {
			registryErr = fmt.Errorf("reading embedded credential registry: %w", err)
			return
		}
		var rd registryData
		if err := json.Unmarshal(data, &rd); err != nil {
			registryErr = fmt.Errorf("parsing embedded credential registry: %w", err)
			return
		}
		registryMap = rd.Credentials
	})
	return registryMap, registryErr
}

func loadCredentialSchema(kind string) ([]byte, error) {
	...
	entry, ok := reg[kind]
	if !ok {
		return nil, fmt.Errorf("no schema found for credential kind %q", kind)
	}
	data, err := schemas.CredentialFS.ReadFile("credentials/" + entry.SchemaFile)
	if err != nil {
...
// catalogProviderEntry groups credential types under a provider.
...
func buildCredentialCatalog() ([]byte, error) {
	catalogOnce.Do(func() {
		reg, err := loadRegistry()
		if err != nil {
			catalogErr = fmt.Errorf("building credential catalog: %w", err)
			return
		}
		...
		if catalogErr != nil {
			catalogErr = fmt.Errorf("marshaling credential catalog: %w", catalogErr)
		}
	})
